...
-- Menu System
local function ShowAnimalMenu()
    local animalOptions = {}
	ExecuteCommand('closeInv')

    for i, animal in ipairs(ANIMAL_TYPES) do
        table.insert(animalOptions, {
            title = animal.label,
            description = "Spawn a " .. animal.label .. " (Health: " .. animal.health .. ") - Will follow you!",
            icon = 'fas fa-paw',
            onSelect = function()
                TriggerEvent('rsg-sanctuary:client:spawnAnimal', i)
            end
        })
    end
    ...
    table.insert(animalOptions, {
        ...
        icon = 'fas fa-whistle',
        onSelect = function()
            TriggerEvent('rsg-sanctuary:client:callAnimals')
        end
    })

    table.insert(animalOptions, {
        title = "Make All Stay",
        description = "Make all your animals stay in place",
        icon = 'fas fa-stop',
        onSelect = function()
            TriggerEvent('rsg-sanctuary:client:makeAllStay')
        end
    })

    lib.registerContext({
        id = 'animal_selection_menu',
        title = 'Animal Management',
        options = animalOptions
    })

    lib.showContext('animal_selection_menu')
end
...
RegisterNetEvent('rsg-sanctuary:client:openAnimalMenu', function()
    ShowAnimalMenu()
end)
